const { query, queryOne, run } = require('../database/db');

exports.listar = (req, res, next) => {
    try {
        const { tipo } = req.query;
        let sql = 'SELECT * FROM categorias WHERE ativo = 1';
        const params = [];

        if (tipo) {
            sql += ' AND tipo = ?';
            params.push(tipo);
        }

        sql += ' ORDER BY nome';
        const categorias = query(sql, ...params);

        res.json({ success: true, data: categorias });
    } catch (err) {
        next(err);
    }
};

exports.criar = (req, res, next) => {
    try {
        const { nome, tipo, cor, icone } = req.body;

        if (!nome || !tipo) {
            return res.status(400).json({
                success: false,
                error: { message: 'Nome e tipo sao obrigatorios.' }
            });
        }

        const result = run(
            'INSERT INTO categorias (nome, tipo, cor, icone) VALUES (?, ?, ?, ?)',
            nome, tipo, cor || '#6B7280', icone || 'tag'
        );

        const categoria = queryOne('SELECT * FROM categorias WHERE id = ?', result.lastInsertRowid);

        res.status(201).json({ success: true, data: categoria });
    } catch (err) {
        next(err);
    }
};

exports.atualizar = (req, res, next) => {
    try {
        const { id } = req.params;
        const { nome, tipo, cor, icone } = req.body;

        run(
            "UPDATE categorias SET nome = ?, tipo = ?, cor = ?, icone = ?, atualizado_em = datetime('now', 'localtime') WHERE id = ?",
            nome, tipo, cor, icone, id
        );

        const categoria = queryOne('SELECT * FROM categorias WHERE id = ?', id);

        res.json({ success: true, data: categoria });
    } catch (err) {
        next(err);
    }
};

exports.desativar = (req, res, next) => {
    try {
        const { id } = req.params;

        run("UPDATE categorias SET ativo = 0, atualizado_em = datetime('now', 'localtime') WHERE id = ?", id);

        res.json({ success: true, message: 'Categoria desativada.' });
    } catch (err) {
        next(err);
    }
};
